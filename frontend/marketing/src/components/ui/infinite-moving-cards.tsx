  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            className="relative w-[250px] max-w-full flex-shrink-0 rounded-2xl border border-b-0 border-foreground  px-6 py-4 sm:w-[300px] md:w-[350px] "
            key={`${item.name}-${idx}`}
          >
            <blockquote className="h-full flex flex-col">
              <span className="relative z-20 text-sm leading-[1.6] font-normal   flex-grow">
                {item.quote}
              </span>
